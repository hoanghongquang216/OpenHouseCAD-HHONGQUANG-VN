...
static void TestLineBounds() {
    ...
    OH_CHECK(box.min.x == 1.0 && box.min.y == -3.0);
    OH_CHECK(box.max.x == 5.0 && box.max.y == 7.0);
}

static void TestCircleBounds() {
    const Circle2d circle{Point2d{10.0, 10.0}, 3.0};
    const auto box = Bounds(circle);
    OH_CHECK(NearlyEqual(box.min.x, 7.0));
    OH_CHECK(NearlyEqual(box.min.y, 7.0));
    OH_CHECK(NearlyEqual(box.max.x, 13.0));
    OH_CHECK(NearlyEqual(box.max.y, 13.0));
}

static void TestAngleOnArcPositiveSweep() {
    const Arc2d arc{Point2d{0.0, 0.0}, 1.0, 0.0, kPi / 2.0};
    OH_CHECK(AngleOnArc(arc, 0.0));          // start, inclusive
    OH_CHECK(AngleOnArc(arc, kPi / 2.0));    // end, inclusive
    OH_CHECK(AngleOnArc(arc, kPi / 4.0));    // middle
    OH_CHECK(!AngleOnArc(arc, kPi));         // outside
    OH_CHECK(!AngleOnArc(arc, -kPi / 4.0));  // outside (before start)
}

static void TestAngleOnArcNegativeSweep() {
    // Clockwise from pi/2 down to 0.
    const Arc2d arc{Point2d{0.0, 0.0}, 1.0, kPi / 2.0, 0.0};
    OH_CHECK(AngleOnArc(arc, kPi / 2.0)); // start
    OH_CHECK(AngleOnArc(arc, 0.0));       // end
    OH_CHECK(AngleOnArc(arc, kPi / 4.0)); // middle
    OH_CHECK(!AngleOnArc(arc, kPi));      // outside
}
...
static void TestAngleOnArcWraparound() {
    ...
    OH_CHECK(AngleOnArc(arc, 0.0));                    // 0 degrees is within [350, 370]
    OH_CHECK(AngleOnArc(arc, 355.0 * kPi / 180.0));     // 355 degrees, within range
    OH_CHECK(!AngleOnArc(arc, 180.0 * kPi / 180.0));    // 180 degrees, well outside
}
...
static void TestArcBoundsWhenNoCardinalCrossed() {
    ...
    OH_CHECK(NearlyEqual(box.min.x, std::min(s.x, e.x)));
    OH_CHECK(NearlyEqual(box.max.x, std::max(s.x, e.x)));
    OH_CHECK(NearlyEqual(box.min.y, std::min(s.y, e.y)));
    OH_CHECK(NearlyEqual(box.max.y, std::max(s.y, e.y)));
}
...
static void TestArcBoundsWhenCrossingTop() {
    ...
    OH_CHECK(NearlyEqual(box.max.y, 10.0)); // center.y (0) + radius (10)
...
static void TestArcBoundsFullCircleMatchesCircleBounds() {
    ...
    OH_CHECK(NearlyEqual(arcBox.min.x, circleBox.min.x));
    OH_CHECK(NearlyEqual(arcBox.min.y, circleBox.min.y));
    OH_CHECK(NearlyEqual(arcBox.max.x, circleBox.max.x));
    OH_CHECK(NearlyEqual(arcBox.max.y, circleBox.max.y));
}
...
static void TestArcBoundsCrossingAllFourCardinalsOffCenter() {
    ...
    OH_CHECK(NearlyEqual(box.min.y, center.y - radius));
