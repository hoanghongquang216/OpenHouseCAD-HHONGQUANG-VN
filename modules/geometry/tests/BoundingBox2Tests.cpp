...
static void TestWidthHeight() {
    ...
    OH_CHECK(Width(box) == 4.0);
    OH_CHECK(Height(box) == 6.0);
}
...
static void TestContainsInclusive() {
    ...
    OH_CHECK(Contains(box, Point2d{5.0, 5.0}));
    OH_CHECK(Contains(box, Point2d{0.0, 0.0}));   // corner, inclusive
    OH_CHECK(Contains(box, Point2d{10.0, 10.0})); // corner, inclusive
    OH_CHECK(!Contains(box, Point2d{-1.0, 5.0}));
    OH_CHECK(!Contains(box, Point2d{5.0, 10.1}));
}
...
static void TestIntersectsOverlapping() {
    ...
    OH_CHECK(Intersects(a, b));
    OH_CHECK(Intersects(b, a)); // symmetric
}

static void TestIntersectsTouchingEdge() {
    const BoundingBox2d a{Point2d{0.0, 0.0}, Point2d{10.0, 10.0}};
    const BoundingBox2d b{Point2d{10.0, 0.0}, Point2d{20.0, 10.0}};
    OH_CHECK(Intersects(a, b)); // touching at x=10 edge counts as intersecting
}
...
static void TestUnion() {
    ...
    OH_CHECK(u.min.x == 0.0 && u.min.y == -2.0);
    OH_CHECK(u.max.x == 10.0 && u.max.y == 5.0);
}

static void TestExpand() {
    BoundingBox2d box = FromPoint(Point2d{5.0, 5.0});
    OH_CHECK(box.min == box.max);

    box = Expand(box, Point2d{0.0, 10.0});
    OH_CHECK(box.min.x == 0.0 && box.min.y == 5.0);
    OH_CHECK(box.max.x == 5.0 && box.max.y == 10.0);

    box = Expand(box, Point2d{8.0, 2.0});
    OH_CHECK(box.min.x == 0.0 && box.min.y == 2.0);
    OH_CHECK(box.max.x == 8.0 && box.max.y == 10.0);
}
