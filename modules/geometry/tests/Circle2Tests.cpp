...
static void TestConstruction() {
    ...
    OH_CHECK(c.center.x == 1.0 && c.center.y == 2.0);
    OH_CHECK(c.radius == 5.0);
}
...
static void TestCircumference() {
    ...
    OH_CHECK(NearlyEqual(Circumference(c), 2.0 * 3.14159265358979323846));
}

static void TestArea() {
    const Circle2d c{Point2d{0.0, 0.0}, 2.0};
    OH_CHECK(NearlyEqual(Area(c), 3.14159265358979323846 * 4.0));
}

static void TestContainsCenterIsInside() {
    const Circle2d c{Point2d{5.0, 5.0}, 3.0};
    OH_CHECK(Contains(c, c.center));
}

static void TestContainsPointOnBoundary() {
    const Circle2d c{Point2d{0.0, 0.0}, 5.0};
    OH_CHECK(Contains(c, Point2d{5.0, 0.0}));
    OH_CHECK(Contains(c, Point2d{0.0, 5.0}));
    OH_CHECK(Contains(c, Point2d{3.0, 4.0})); // 3-4-5 triangle, exactly on boundary
}

static void TestContainsPointOutside() {
    const Circle2d c{Point2d{0.0, 0.0}, 5.0};
    OH_CHECK(!Contains(c, Point2d{5.1, 0.0}));
    OH_CHECK(!Contains(c, Point2d{10.0, 10.0}));
}

static void TestContainsPointInside() {
    const Circle2d c{Point2d{0.0, 0.0}, 5.0};
    OH_CHECK(Contains(c, Point2d{1.0, 1.0}));
    OH_CHECK(Contains(c, Point2d{0.0, 0.0}));
}

static void TestOffCenterCircle() {
    const Circle2d c{Point2d{10.0, -10.0}, 2.0};
    OH_CHECK(Contains(c, Point2d{10.0, -10.0}));
    OH_CHECK(Contains(c, Point2d{11.0, -10.0}));
    OH_CHECK(!Contains(c, Point2d{13.0, -10.0}));
}
